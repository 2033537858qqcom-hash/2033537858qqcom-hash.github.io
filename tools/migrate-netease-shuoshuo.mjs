...
function cleanMsg (msg) {
  ...
  t = t.replace(/\n?(?:#[^\n#]+#\s*)+$/g, '').trim()
  t = t.replace(/[ \t]+\n/g, '\n')
  // YAML 1.1 禁止的非打印字符（含零宽、BOM、控制符）；保留 \n \t
  t = t.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F\u200B-\u200F\u2028\u2029\uFEFF]/g, '')
  // 网易云个别条目自带 U+FFFD，按上下文补全
  const fixups = [
    [/\uFFFD+途一片迷茫/g, '前途一片迷茫'],
    [/结\uFFFD+了/g, '结束了'],
    [/才\uFFFD+年/g, '才两年'],
...
for (const e of raw) {
  ...
  let content = msg
  if (e.song && e.song.name) {
    content += `\n\n—— 配乐：${e.song.name}${e.song.artists ? ' / ' + e.song.artists : ''}`
  }

  kept.push({
    content,
    date: e.date,
    tags: extractTags(msg, e.song),
    source: 'netease',
    id: e.id
  })
}
...
const all = [...blogNative, ...kept].sort((a, b) => (a.date < b.date ? 1 : -1))
